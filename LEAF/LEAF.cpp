...
void parseBOWTIE(char buf[], unsigned int & targetID, unsigned int & targetStart, unsigned int & targetEnd, unsigned int & targetGap, unsigned int & sourceID, unsigned int & sourceStart, unsigned int & sourceEnd, unsigned int & sourceGap, unsigned int & sourceSize, vector<Segment> & seg, unsigned int & fr)
{
...
void parseBLAT(char buf[], unsigned int & targetID, unsigned int & targetStart, unsigned int & targetEnd, unsigned int & targetGap, unsigned int & sourceID, unsigned int & sourceStart, unsigned int & sourceEnd, unsigned int & sourceGap, unsigned int & sourceSize, vector<Segment> & seg, unsigned int & fr, unsigned int & targetSize)
{
        char targetIDBuf[20] = {'\0'}, targetStartBuf[20] = {'\0'}, targetEndBuf[20] = {'\0'}, targetGapBuf[20] = {'\0'}, sourceIDBuf[20] = {'\0'}, sourceStartBuf[20] = {'\0'}, sourceEndBuf[20] = {'\0'}, sourceGapBuf[20] = {'\0'}, sourceSizeBuf[20] = {'\0'}, blockBuf[20] = {'\0'}, targetSizeBuf[20] = {'\0'};
        int item = 0, i, j0 = 0, j1 = 0, j2 = 0, j3 = 0, j4 = 0, j5 = 0, j6 = 0, j7 = 0, j8 = 0, j9 = 0, k, sp, j10 = 0;
        Segment s;
...
void loadContiAli(ifstream & ca, vector<Seq> & contigs, int chromosomeID)
{
	...
	if(ca.is_open())
	{
		while(ca.good())
		{
			...
			parseBLAT(buf, targetID, targetStart, targetEnd, targetGap, sourceID, sourceStart, sourceEnd, sourceGap, sourceSize, segs, fr, targetSize);
...
//		cout << "*************************" << endl;
        ...
        for(sp = 0; sp < seqs.size(); sp ++)
        {
		if(seqs[sp].outputted == 1)
		{
	                out << ">" << sp << endl;
	                for(np = 0; np < seqs[sp].nucleotides.size(); np ++)
	                {
	                        out << seqs[sp].nucleotides[np];
	                        if((np + 1) % 60 == 0 || np == seqs[sp].nucleotides.size() - 1)
	                                out << endl;
	                }
		}
        }
...
#else
                        if((double) (sourceEnd1 - sourceStart1 - sourceGap1) / (sourceEnd1 - sourceStart1) >= THRESHOLD && (double) (targetEnd1 - targetStart1 - targetGap1) / (targetEnd1 - targetStart1) >= THRESHOLD && (double) (sourceEnd2 - sourceStart2 - sourceGap2) / (sourceEnd2 - sourceStart2) >= THRESHOLD && (double) (targetEnd2 - targetStart2 - targetGap2) / (targetEnd2 - targetStart2) >= THRESHOLD)// 0.8 to 0 for test
#endif
			{
...
///	if(k1.contigID != k2.contigID || k1.contigID == -1 || k2.contigID == -1 || (k1.contigID == k2.contigID && (abs((int)(k1.contigOffset - k2.contigOffset)) <= 5 * EP || k1.contigOffset == -1 || k2.contigOffset == -1)) && 
///	(k1.contigID0 != k2.contigID0 || k1.contigID0 == -1 || k2.contigID0 ==-1 || (k1.contigID0 == k2.contigID0 && abs((int)(k1.contigOffset0 - k2.contigOffset0)) <= 2 * insertVariation + 5 * EP /*+ mrl*/ || k1.contigOffset0 == -1 || k2.contigOffset0 == -1)) && 
///	(k1.chromosomeID0 == k2.chromosomeID0 || k1.chromosomeID0 == -1 || k2.chromosomeID0 == -1) && 
///	(abs((int)(k1.chromosomeOffset0 - k2.chromosomeOffset0)) <= 2 * insertVariation + 5 * EP /*+ mrl*/ || k1.chromosomeOffset0 == -1 || k2.chromosomeOffset0 == -1) && // the latter "-1"s are possible for SDs
///	1 /*getDiff(k1.s, k2.s) < k1.s.size() / 2*/)// the easiest way to tolerate errors is to remove k-mer comparison
//must also consider read length for abs((int)(k1.chromosomeOffset0 - k2.chromosomeOffset0)) <= 2 * (distanceHigh - distanceLow) / 2

//        if((k1.contigID == k2.contigID || k1.contigID == -1 || k2.contigID == -1) && (abs((int)(k1.contigOffset - k2.contigOffset)) <= 3 * SD || k1.contigOffset == -1 || k2.contigOffset == -1) &&
//      ((k1.contigID0 = k2.contigID0 || k1.contigID0 == -1 || k2.contigID0 ==-1) && abs((int)(k1.contigOffset0 - k2.contigOffset0)) <= 2 * (distanceHigh - distanceLow) / 2 + 3 * SD || k1.contigOffset0 == -1 || k2.contigOffset0 == -1) &&
//      (k1.chromosomeID0 == k2.chromosomeID0 || k1.chromosomeID0 == -1 || k2.chromosomeID0 == -1) &&
//      (abs((int)(k1.chromosomeOffset0 - k2.chromosomeOffset0)) <= 2 * (distanceHigh - distanceLow) / 2 || k1.chromosomeOffset0 == -1 || k2.chromosomeOffset0 == -1) && // the latter "-1"s are possible for SDs
//      1 /*getDiff(k1.s, k2.s) < k1.s.size() / 2*/)// the easiest way to tolerate errors is to remove k-mer comparison

	if(
	(k1.contigID == -1 || k2.contigID == -1 || (k1.contigID != -1 && k1.contigID == k2.contigID && abs((int)(k1.contigOffset - k2.contigOffset)) <= 5 * EP) || (k1.contigID != -1 && k2.contigID != -1 && k1.contigID != k2.contigID) ) &&
// it is hard to judge if two overlapping contigs should be joined or are just repetitive. Here I find it performs better to join them.
	(k1.contigID0 == -1 || k2.contigID0 == -1 || (k1.contigID0 != -1 && k1.contigID0 == k2.contigID0 && abs((int)(k1.contigOffset0 - k2.contigOffset0)) <= 2 * insertVariation + 5 * EP) || (k1.contigID0 != -1 && k2.contigID0 != -1 && k1.contigID0 != k2.contigID0) ) &&
...
//			cout << endl << "-------CONNECT COMPARE--------" << endl;
//			cout << np << "vs" << genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].next.size() << endl;
//			cout << genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset << "vs" << genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset << endl;
//			cout << genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 << "vs" << genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 << endl;
//			cout << "------------------------------" << endl << endl;
			if(np == genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].next.size() ////&& 

///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigID != genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID ||
...
///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigID == genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID &&
///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset == -1 || 
///			genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset == -1 || 
///			genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset - genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset <= 5 * EP))) && 
///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigID0 != genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID0 ||
...
///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigID0 == genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID0 &&
///			(genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 == -1 || 
///			genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 == -1 || 
///			abs((int)(genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 - genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0)) <= 2 * insertVariation + 5 * EP))))
...
//                        (genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset == -1 ||
//                        genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset == -1 ||
//                        genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset - genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset <= 3 * SD) &&
//                        (genome[nextID][nextOffset].kMer[nextItem[nip]].contigID0 == -1 ||
//                        genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID0 == -1 ||
//                         genome[nextID][nextOffset].kMer[nextItem[nip]].contigID0 == genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigID0) &&
//
//                        (genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 == -1 ||
//                        genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 == -1 ||
//                        genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 - genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 <= 2 * (distanceHigh - distanceLow) / 2 + 3 * SD))
...
//				cout << "keep [" << chromosomeID << ", " << chromosomeOffset << ", " << cip << ", " << np << "] to [" << nextID << ", " << nextOffset << ", " << nip << "]" << endl;
			...
			//previous.previousID = chromosomeID;
			//previous.previousOffset = chromosomeOffset;
			//previous.previousItem = chromosomeItem[cip];
			//for(pp = 0; pp < genome[nextID][nextOffset].kMer[nextItem[nip]].previous.size(); pp ++)
				//if(previousCompatible(previous, genome[nextID][nextOffset].kMer[nextItem[nip]].previous[pp]))
					//break;
			//if(pp == genome[nextID][nextOffset].kMer[nextItem[nip]].previous.size() && 
			...
			//genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 == -1 || 
			//genome[nextID][nextOffset].kMer[nextItem[nip]].contigOffset0 - genome[chromosomeID][chromosomeOffset].kMer[chromosomeItem[cip]].contigOffset0 <= 2 * (distanceHigh - distanceLow) / 2 + 3 * SD))
				//genome[nextID][nextOffset].kMer[nextItem[nip]].previous.push_back(previous);
		}
}
...
//			startIDBak = contigs[cp].startID;
		...
		while(contigs[cp].startID0 == contigs[cp].endID0 && cont)
		{
			...
			for(cp0 = cp + 1; cp0 < contigs.size(); cp0 ++)
			{
				...
				{
					for(i = 0; contigs[cp0].startOffset > contigs[cp].endOffset && i < contigs[cp0].startOffset - contigs[cp].endOffset - 1; i ++)
//						scaffolds[scaffolds.size() - 1].push_back('N');
						scaffolds[scaffolds.size() - 1].push_back(genome[0][contigs[cp].endOffset + i + 1].nucleotide);// changed to save space
...
void refinement(ofstream & e, ofstream & r)
{
	...
	{
		s = "blat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_initial_contigs." + itoa(i) + ".fa -noHead tmp/_initial_contigs_extended_contigs." + itoa(i) + ".psl > blat_doc.txt";
		system(s.c_str());
	}
	...
	for(i = 0; i < numChromosome; i ++)
	{
		...
		s = "tmp/_initial_contigs_extended_contigs." + itoa(i) + ".psl";
		ps.open(s.c_str());
		if(ps.is_open())
		{
			while(ps.good())
			{
				ps.getline(buf, 50000);
				if(buf[0] == 0) break;
...
void formalizeInput(ifstream & in1, ifstream & in2, string file)
{
	...
	in2.seekg(0);
        ...
        if(in1.is_open() && in2.is_open())
        {
                ...
                {
                        in1.getline(buf1, 500);
			in2.getline(buf2, 500);
			...
			else if(buf1[0] == 0 && buf2[0] != 0 || buf1[0] != 0 && buf2[0] == 0)
			{
				...
				in2.getline(buf2, 500);
                                out << ">" << seqID << endl;
                                for(i = 0; i < in1.gcount() - 1; i ++)
                                        out << buf1[i];
                                out << endl;
				out << ">" << seqID ++ << endl;
				for(i = 0; i < in2.gcount() - 1; i ++)
					out << buf2[i];
				out << endl;
                        }
...
void * task0(void * arg)
{
	Insert ins;
	string command;
	stringstream distanceLowStr, distanceHighStr, nStr;
	int chromosomeID;
...
void * task1(void * arg)
{
	int chromosomeID, numChromosomes;
	string command;

	numChromosomes = *(int *) arg;
	for(chromosomeID = 0; chromosomeID < numChromosomes; chromosomeID ++)
	{
		command = "blat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl > blat_doc.txt";
		system(command.c_str());
	}
}

void nonParallelMap(int distanceLow, int distanceHigh, int numChromosomes)
{
        string command;
...
void parallelMap(int distanceLow, int distanceHigh, int numChromosomes)
{
	pthread_t t0, t1;
	Insert ins;
	...
	if(pthread_create(&t0, NULL, task0, &ins) != 0) {nonParallelMap(distanceLow, distanceHigh, numChromosomes); return;}
	if(pthread_create(&t1, NULL, task1, &numChromosomes) != 0) {nonParallelMap(distanceLow, distanceHigh, numChromosomes); return;}
...
void print()
{
	cout << "LEAF --read1 reads_1.fa --read2 reads_2.fa --contig contigs.fa --genome genome.fa --distanceLow distanceLow --distanceHigh distancehigh --extendedContig extendedContigs.fa --remainingContig remainingContigs.fa [--kMer k --insertVariation insertVariation --covereage coverage --noAlignment --part p]" << endl;
	cout << "Inputs:" << endl;
...
//	mrl = mrl1 > mrl2 ? mrl1 : mrl2;
	...
	if(tagNoAlignment == 1)
	{
		...
		parallelMap(distanceLow, distanceHigh, numChromosomes);
		endAlign = time(NULL);
